...
fn test_nested_comments() {
    ...
    assert!(
        out.contains("42"),
        "nested comment should be ignored, got: {out}"
    );
}
...
fn test_deeply_nested_comments() {
    ...
    assert!(
        out.contains("99"),
        "deeply nested comment should be ignored, got: {out}"
    );
}
...
fn test_mul_before_add() {
    ...
    assert!(
        out.contains("14"),
        "mul should bind tighter than add, got: {out}"
    );
}
...
fn test_parentheses_override() {
    ...
    assert!(
        out.contains("20"),
        "parentheses should override precedence, got: {out}"
    );
}
...
fn test_power_precedence() {
    ...
    assert!(
        out.contains("11"),
        "power should bind tighter than add, got: {out}"
    );
}
...
fn test_empty_list() {
    ...
    assert!(
        out.contains("{}"),
        "empty list should print as {{}}, got: {out}"
    );
}
...
fn test_nested_empty_lists() {
    ...
    assert!(
        out.contains("{}"),
        "nested empty lists should contain {{}}, got: {out}"
    );
}
...
fn test_function_basic() {
    ...
    assert!(
        out.contains("6"),
        "Function[x, x+1][5] should be 6, got: {out}"
    );
}
...
fn test_function_multiply() {
    ...
    assert!(
        out.contains("6"),
        "Function[x, x*2][3] should be 6, got: {out}"
    );
}
...
fn test_function_multi_param() {
    ...
    assert!(
        out.contains("7"),
        "Function[{{x,y}}, x+y][3,4] should be 7, got: {out}"
    );
}
...
fn test_chained_rules() {
    ...
    assert!(
        out.contains("y"),
        "first rule x->y should apply, got: {out}"
    );
}
...
fn test_replace_repeated() {
    ...
    assert!(
        out.contains("z"),
        "replace repeated should converge to z, got: {out}"
    );
}
...
fn test_match_q_string() {
    ...
    assert!(
        out.contains("True"),
        "\"hi\" should match _String, got: {out}"
    );
}
...
fn test_match_q_wrong_type() {
    ...
    assert!(
        out.contains("False"),
        "1 should not match _String, got: {out}"
    );
}
